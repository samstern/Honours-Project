...
con = mdb.connect('localhost', 'root', '', 'House_Sets_Sam');
with con:

    # Use a Dictionary Cursor
    cur = con.cursor(mdb.cursors.DictCursor)
    # Get the household ID numbers
    cur.execute("Select Distinct Household from hes_database.total_profiles where household=203445")
    # Retrieve them all
    HouseNumbers = cur.fetchall()

    # Tables can't be called numbers, use s+number as temporary holding name
    for row in HouseNumbers:
        tic = timeit.default_timer()
        HouseNumber1 = row["Household"]
        HouseNumber2 = 's' + HouseNumber1
        ...
        FROM house_sets_sam.filtered_data hes \
        ...
        into_dt_Id1 = "INSERT INTO ds_%s_Id1 (HouseID, Data, Date, Hour, Minute) \
        Select HouseID, sum(Data), Dates, Hour, Minute \
        FROM %s \
        WHERE IntervalID = 1 \
        Group By Dates,Hour,Minute" % (HouseNumber1, HouseNumber2)

        into_dt_Id2 = "INSERT INTO ds_%s_Id2 (HouseID, Data, Date, Hour, Minute) \
        Select HouseID, sum(Data), Dates, Hour, Minute \
        FROM %s \
        WHERE IntervalID = 2 \
        Group By Dates,Hour,Minute" % (HouseNumber1, HouseNumber2)

        into_dt_Id3 = "INSERT INTO ds_%s_Id3 (HouseID, Data, Date, Hour, Minute) \
        Select HouseID, sum(Data), Dates, Hour, Minute \
        FROM %s \
        WHERE IntervalID = 3 \
        Group By Dates,Hour,Minute" % (HouseNumber1, HouseNumber2)
