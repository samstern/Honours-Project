...
def getStartDay(cur, tables):
    ...
    for table in tables:
        ...
        getStartDay = "select dayofweek(Date) from Ten_Mins_Subset.%s order by date desc limit 1" %(Table_Name)
...
con = mdb.connect('localhost', 'root', '', 'Four_Weeks_Subset');
with con:
    ...
    for table in tables:
        ...
        while numDays<28:
                ...
                if nextDay == ():
                    ...
                    insertData = "INSERT INTO %s (SELECT * FROM %s WHERE Date=(%s) order by Hour, Minute)"%(Table_Name, Table_Name,getDay)
