...
def GetInfoId1(cursor):
    IDs = []
    cursor.execute("Show Tables FROM Ten_Mins_Subset LIKE '%Id1'")
...
def GetInfoId2(cursor):
    IDs = []
    cursor.execute("Show Tables FROM Ten_Mins_Subset LIKE '%Id2'")
    tables = cursor.fetchall()
    for tab in tables:
        IDs.append(tab)
    return IDs
...
def GetInfoId3(cursor):
    IDs = []
    cursor.execute("Show Tables FROM Ten_Mins_Subset LIKE '%Id3'")
    tables = cursor.fetchall()
    for tab in tables:
        IDs.append(tab)
    return IDs
