function ...
    WriteMyKNNFile(ChosenFeatures,CurrentMatrixName,BinMulti,...
    Pm,PM,Rm,RM,F1M,F1m,Ac,Neighbours,ConfMat)

fileName = sprintf('%s_%s_%s_KNN',ChosenFeatures,CurrentMatrixName,BinMulti);
write_fileName = fopen(fileName,'w');

fprintf(write_fileName,'%s_%s_%s_KNN \n',ChosenFeatures,CurrentMatrixName,BinMulti);
...
fprintf(write_fileName, 'Number of Neighbours: %d \n',Neighbours);
