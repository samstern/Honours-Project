...
def StdAvgPerHalfHour(cursor,Table,HouseNumber):
    StdvAvgHourly = []
    HouseNumber = HouseNumber[0].split('_')[0]
    StdvAvgHourly.append(HouseNumber)
    Hours = range(0,24)
    ...
    for hour in Hours:
        for minute in Minutes:
            ...
            StdvAvgHourly.append([hour,av,stdv,minute])
    return StdvAvgHourly
...
def AvgPerHalfHourDay(cursor,Table,HouseNumber):
    AvgHalfHourDaily = []
    HouseNumber = HouseNumber[0].split('_')[0]
    AvgHalfHourDaily.append(HouseNumber)
    Hours = range(0,24)
    mins = [0,30]
    Day_of_the_week = "date_format(Date, '%W')"
    Days = ["'Monday'", "'Tuesday'", "'Wednesday'", "'Thursday'", \
    ...
    for day in Days:
        for hour in Hours:
            for minute in mins:
                query = "select AVG(Data) as AvgHourDay \
                from %s where Hour = %s AND Minute = %s and %s = %s GROUP BY HOUR" % (Table,hour,minute, Day_of_the_week, day)
                cursor.execute(query)
                avd = cursor.fetchone()
                if avd is None:
                    avd = 0
                    ...
                    avd = float(avd[0])
                    AvgHalfHourDaily.append([day,hour,avd,minute])
    return AvgHalfHourDaily
...
def StdAvgPerHalfHourDay(cursor,Table,HouseNumber):
    StdAvgHourDaily = []
    HouseNumber = HouseNumber[0].split('_')[0]
    StdAvgHourDaily.append(HouseNumber)
    Hours = range(0,24)
    ...
    for day in Days:
        for hour in Hours:
            for minute in Minutes:
                query = "select AVG(Data) as AvgHalfHourDay, STD(Data) as StdHalfHourDay \
                from %s where Hour = %s AND Minute = %s and %s = %s GROUP BY HOUR" % (Table,hour,minute, Day_of_the_week, day)
                cursor.execute(query)
